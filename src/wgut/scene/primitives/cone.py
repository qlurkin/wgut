...
from wgut.cgmath import normalize, vec, vec3
...
def cone(radius=1.0, height=2.0, segments=32) -> StaticMesh:
    ...
    for i in range(segments + 1):
        ...
        position = vec3([x, base_y, z, 1.0])
        normal = normalize([x, radius / height, z])

        uv = vec([i / segments, 0.0])
    ...
    for i in range(segments):
        ...
        position = vec([0.0, tip_y, 0.0, 1.0])
        uv = vec([0.5, 1.0])
    ...
    for i in range(segments + 1):
        ...
        position = vec([x, base_y, z, 1.0])
        normal = vec3([0, -1, 0])
        uv = vec([0.5 + x / (2 * radius), 0.5 + z / (2 * radius)])
        tangent = vec3([1, 0, 0])
    ...
    positions.append(vec([0, base_y, 0, 1.0]))
    normals.append(vec3([0, -1, 0]))
    uvs.append(vec([0.5, 0.5]))
